...
// 内部变量，用于存储从 main.js 传来的回调
var onStartCallback = null;
var onPreprocessCallback = null;
...
// ===                 工作流编辑器                             ===
// ===================================================================
function openWorkflowEditor(schemeIndex, workflowSchemes, stepMap, onUpdate) {
    var currentEditingWorkflow = workflowSchemes.schemes[schemeIndex].slice();
...
function openCoordinateSettings() {
        ...
        <vertical padding="20dp">
            ...
            <ScrollView layout_weight="1" marginTop="16dp">
                <vertical>
                    ...
                    <horizontal gravity="center_vertical" marginBottom="8dp">
                    <text text="删除图层按钮" layout_weight="1" textColor="#e74c3c"/>
                    <input id="coords_delete_layer" w="150dp" h="40dp"/>
                    </horizontal>//新增
    ...
    var allInputs = [
        coordsUi.coords_apply, coordsUi.coords_save, coordsUi.coords_confirm_save, coordsUi.coords_delete_layer,//新增
        coordsUi.coords_next_batch_folder,
    ...
    function populateInputs(coords) {
        ...
        coordsUi.coords_delete_layer.setText(singleCoordToStr(coords.ps_deleteLayerBtn));//新增
    ...
    coordsUi.btn_save_coords.on("click", function() {
        try {
            ...
            newCoords.ps_deleteLayerBtn = robustStrToSingleCoord(coordsUi.coords_delete_layer.getText()) || defaultCoords.ps_deleteLayerBtn;//新增
...
// 初始化UI
uiModule.init = function(initialData, startCb, preprocessCb) {
    onStartCallback = startCb;
    onPreprocessCallback = preprocessCb;

    var scriptInfo = initialData.scriptInfo;
    var workflowSchemes = initialData.workflowSchemes;
    var stepMap = initialData.stepMap;
                    ...
                    <vertical padding="8dp">
                        ...
                        <card w="*" margin="8dp 4dp" cardCornerRadius="8dp" cardElevation="2dp">
                            <vertical padding="16dp" id="container_performance">
                                ...
                                <vertical id="container_fast_input_settings" marginLeft="8dp" marginTop="10dp" visibility="gone">
                                     <horizontal gravity="center_vertical">
                                        <vertical layout_weight="1">
                                ...
                                <horizontal gravity="center_vertical" id="container_slow_motion" marginTop="10dp" visibility="gone">
                                    <vertical layout_weight="1">
    ...
    function collectAllSettings(taskType) {
        ...
        var baseWorkflow = workflowSchemes.schemes[workflowSchemes.active_scheme_index];
        var finalWorkflow = baseWorkflow.slice();
    ...
    schemeButtons.forEach(function(button, index) {
        button.on('click', function() { switchActiveScheme(index); });
    });

    ui.btn_edit_workflow.on('click', function() {
        openWorkflowEditor(workflowSchemes.active_scheme_index, workflowSchemes, stepMap, updateWorkflowDisplay);
    });
    ...
    ui.btn_show_manual.on('click', function() { dialogs.alert("使用说明", scriptInfo.userManual); });
    ui.btn_show_changelog.on('click', function() { dialogs.alert("更新日志 (v" + scriptInfo.version + ")", scriptInfo.changelog); });
    ...
    ui.btn_start.on('click', function() {
        ...
        if (currentTime - lastClickTimeStart < doubleClickDelay) {
            lastClickTimeStart = 0;
            if (onStartCallback) {
                var settings = collectAllSettings('main');
                onStartCallback(settings);
            }
        } else {
            lastClickTimeStart = currentTime;
            toast("再按一次启动主流程");
        }
    });
    ui.btn_preprocess.on('click', function() {
        ...
        var currentTime = new Date().getTime();
        if (currentTime - lastClickTimePreprocess < doubleClickDelay) {
            lastClickTimePreprocess = 0;
            if (onPreprocessCallback) {
                var settings = collectAllSettings('preprocess');
                onPreprocessCallback(settings);
            }
        } else {
            lastClickTimePreprocess = currentTime;
            toast("再按一次启动预处理");
        }
    });
    ...
    // 让UI可见
    ui.root_layout.attr("visibility", "visible");
};
